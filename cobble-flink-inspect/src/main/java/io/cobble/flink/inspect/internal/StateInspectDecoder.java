...
public final class StateInspectDecoder {
    ...
    public static DecodedRow decode(InspectTarget target, byte[] rowKey, byte[][] columns) {
    ...
    private static boolean hasCompleteFullyClasslessSemanticParts(
        ...
        // Value-like states: value serializer.
    ...
    public static byte[] encodeStateKeyPrefix(
    ...
    public static boolean mapKeyBytesStartsWith(InspectTarget target, byte[] rowKey, byte[] prefix)
            throws IOException {
        if (target == null
                || target.schema == null
    ...
    private static SemanticDecodeResult decodeTimerSemanticParts(
        ...
        try {
            ...
            ByteArrayInputStream bytes = new ByteArrayInputStream(rowKey);
            DataInputViewStreamWrapper input = new DataInputViewStreamWrapper(bytes);
            long rawTimestamp = input.readLong();
            long timestamp = MathUtils.flipSignBit(rawTimestamp);
            // Preserve the timestamp even when the key/namespace boundary cannot be determined.
            output.put("timestamp", DisplayLong.forJson(timestamp));

            int keyStart = rowKey.length - bytes.available();
            // Consume the key bytes to find the namespace boundary. This uses the restored
            // serializer; if it fails (e.g. FULLY_CLASSLESS Avro without flink-avro), the
            ...
            namespaceStart = rowKey.length - bytes.available();
            int end;
            if (isVoidNamespaceSerializer(schema.namespaceSerializer())) {
                try {
                    requireLength(
    ...
    public static void validateSemanticPartFilter(
    ...
    public static int keyGroupForSemanticStateKey(
    ...
    private static Object renderSemanticScalar(Object value) throws IOException {
        ...
        if (value instanceof byte[]) {
            return InspectJsonValues.bytesJson((byte[]) value);
        }
        ...
        if (fixedBytes != null) {
            return InspectJsonValues.bytesJson(fixedBytes);
        }
     ...
     * always restores the live serializer.
     *
     * @return the decoded value, or {@code null} if the bytes are null.
     * @throws IOException if classless decode fails on a {@code FULLY_CLASSLESS} descriptor, or if
     *     the restored-serializer fallback also fails.
     */
    private static Object deserializeSemanticValue(
            SerializerInspectSchema serializerSchema, byte[] bytes) throws IOException {
        if (bytes == null) {
            return null;
        }
    ...
    public static String simpleSerializerName(String className) {
    ...
    private static Object render(Object value, byte[] rawBytes, TypeSerializer<Object> serializer)
        ...
        if (fallbackBytes != null) {
            return InspectJsonValues.bytesJson(fallbackBytes);
        }
    ...
    public static final class DecodedRow {
        public final Map<String, Object> decodedKey;
        public final Object decodedValue;
        public final Map<String, Object> decodedParts;
        public final String decodeError;
        public final List<DecodeIssue> decodeIssues;
    ...
    public static final class DecodeIssue {
        public final String part;
        public final DecodeIssueKind kind;
     ...
     */
    public static final class DecodeIssueCollector {
