 ...
 */
public final class UserClasspath implements AutoCloseable {
    ...
    public static UserClasspath empty() {
        ClassLoader parent = UserClasspath.class.getClassLoader();
     ...
     */
    public static UserClasspath create(List<String> paths) {
        ...
        for (String raw : paths) {
            ...
            if (!file.exists()) {
                throw new InspectInputException("--user-jar path does not exist: " + path);
            }
            if (file.isDirectory()) {
                File[] children = file.listFiles((dir, name) -> name.endsWith(".jar"));
                if (children == null) {
                    throw new InspectInputException("cannot read directory: " + path);
                }
