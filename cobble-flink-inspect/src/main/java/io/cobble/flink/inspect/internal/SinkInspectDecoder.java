...
public final class SinkInspectDecoder {

    private SinkInspectDecoder() {}

    /** Encodes a leading subset of the ordered sink primary key for prefix scans. */
    public static byte[] encodeKeyPrefix(InspectTarget target, List<String> values)
            throws IOException {
    ...
    public static DecodedRow decode(
            InspectTarget target, byte[] key, byte[][] columns, int[] projection) {
        if (target == null || target.sinkSchema == null) {
    ...
    public static Object parseFieldInput(LogicalType logicalType, String text) throws IOException {
    ...
    private static Object render(Object value, byte[] rawBytes) {
        ...
        if (rawBytes != null) {
            return InspectJsonValues.bytesJson(rawBytes);
        }
        return value.toString();
    }

    private static Object fallbackBytes(byte[] bytes) {
        return bytes == null ? null : InspectJsonValues.bytesJson(bytes);
    }
    ...
    public static final class DecodedRow {
        public final List<Map<String, Object>> decodedKey;
        public final List<Map<String, Object>> decodedColumns;
